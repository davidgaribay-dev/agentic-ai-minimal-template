...
**CRITICAL: ALWAYS use autogenerate** - NEVER write migration files manually.
...
### Post-Autogenerate Review

After autogenerate, review the file and add if needed:

1. **pgvector import** (for vector columns):
   ```python
   import pgvector.sqlalchemy
   ```

2. **PostgreSQL extensions** (at start of upgrade()):
   ```python
   def upgrade() -> None:
...
3. **Renamed columns**: Autogenerate shows drop + add. Manually change to:
   ```python
...
- LangGraph `checkpoint_*` tables show as "removed" - ignore these warnings
- Complex data migrations must be added manually
- Custom SQL (triggers, functions) not detected
...
```bash
# 1. Generate migration (ALWAYS use autogenerate)
uv run alembic revision --autogenerate -m "description"

# 2. Review generated file - add if needed:
#    - import pgvector.sqlalchemy (for vector columns)
#    - CREATE EXTENSION statements (for new databases)
#    - Fix renamed columns (autogenerate shows drop + add)

# 3. Apply migration
uv run alembic upgrade head

# 4. Verify it can be rolled back
