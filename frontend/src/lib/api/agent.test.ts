...
describe("agentApi.chatStream", () => {
  ...
  describe("title events", () => {
    it("parses title events correctly", async () => {
      const events = [
        { event: "title", data: { title: "Generated Title", conversation_id: "conv-123" } },
      ]
      globalThis.fetch = vi.fn().mockResolvedValue(createMockSSEResponse(events))
  ...
  describe("error events", () => {
    ...
    it("handles error in data without event type", async () => {
      const events = [
        { data: { error: "Error message from data" } },
      ]
      globalThis.fetch = vi.fn().mockResolvedValue(createMockSSEResponse(events))
  ...
  describe("tool_approval events", () => {
    it("parses tool approval events correctly", async () => {
      ...
      const approvalEvent = streamEvents.find((e) => e.type === "tool_approval")
      ...
      if (approvalEvent?.type === "tool_approval") {
        ...
        expect(approvalEvent.data.tool_description).toBe("Search for relevant documents")
  ...
  describe("sources events", () => {
    it("parses sources events correctly", async () => {
      const events = [
        createSourcesEvent([
          { content: "Source content 1", source: "doc1.md", relevanceScore: 0.95 },
          { content: "Source content 2", source: "doc2.md", relevanceScore: 0.85 },
        ]),
      ]
      globalThis.fetch = vi.fn().mockResolvedValue(createMockSSEResponse(events))
  ...
  describe("guardrail_block events", () => {
    it("parses guardrail block events correctly", async () => {
      ...
      if (blockEvent?.type === "guardrail_block") {
        expect(blockEvent.data.message).toBe("Content blocked for policy violation")
  ...
  describe("complete stream", () => {
    it("handles a complete chat stream with all event types", async () => {
      const events = createCompleteChatStream("Hello, how can I help?", "conv-123", "Chat Title")
      globalThis.fetch = vi.fn().mockResolvedValue(createMockSSEResponse(events))
  ...
  describe("request formatting", () => {
    it("includes auth token in request headers", async () => {
      ...
      globalThis.fetch = vi.fn().mockImplementation(async (_url, init) => {
        ...
        return createMockSSEResponse([{ event: "done", data: { conversation_id: "conv-123" } }])
      })
