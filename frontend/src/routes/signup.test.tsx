  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center bg-background p-4">
      <div className="relative w-full max-w-[360px]">
        ...
        {step === "email" && (
          <form {...testId("signup-form-email")} onSubmit={handleEmailSubmit}>
            <Input
              ...
              autoComplete="email"
            />
            <Button {...testId("signup-continue-button")} type="submit" className="mt-4 w-full">
        ...
        {/* Step 2: Password */}
        {step === "password" && (
          <form {...testId("signup-form-password")} onSubmit={handlePasswordSubmit}>
            ...
            <Button type="button" variant="link" onClick={() => setStep("email")} className="mt-2 w-full">
              {t("com_back")}
            </Button>
          </form>
        )}
        ...
        {step === "name" && (
          <form onSubmit={handleNameSubmit}>
            ...
            <Button type="button" variant="link" onClick={() => setStep("password")} className="mt-2 w-full">
              {t("com_back")}
            </Button>
          </form>
        )}
        ...
        {step === "workspace" && (
          <form onSubmit={handleWorkspaceSubmit}>
            ...
            <Button type="button" variant="link" onClick={() => setStep("name")} className="mt-2 w-full">
              {t("com_back")}
            </Button>
          </form>
        )}
        ...
        {step === "team" && (
          <form {...testId("signup-form-team")} onSubmit={handleTeamSubmit}>
            ...
            >
              {register.isPending ? t("auth_creating") : t("auth_create_workspace")}
            </Button>
            <Button
              type="button"
              variant="link"
...
describe("Signup Page", () => {
  ...
  describe("Full Wizard Flow", () => {
    it("navigates through all steps to team form", async () => {
      ...
      await user.type(screen.getByPlaceholderText(/workspace name/i), "My Workspace")
      await user.click(screen.getByRole("button", { name: /continue/i }))
    ...
    it("requires workspace name before proceeding to team step", async () => {
      ...
      await waitFor(() => expect(screen.getByTestId("signup-password-input")).toBeInTheDocument())
      await user.type(screen.getByTestId("signup-password-input"), "password123")
      await user.click(screen.getByRole("button", { name: /continue/i }))
      ...
      await waitFor(() => expect(screen.getByPlaceholderText(/workspace name/i)).toBeInTheDocument())
      await user.click(screen.getByRole("button", { name: /continue/i }))
